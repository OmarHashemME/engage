...
const LessonLayout: React.FC<LessonLayoutProps> = ({ 
  ...
  // Determine effective user level for this lesson's activity
  const effectiveUserLevel = levelService.getEffectiveLevel(USER_DATA, lesson.activityType);

  // Default mode logic: A1/A2 -> Support (Bilingual), B1+ -> Challenge (Immersion)
  const [mode, setMode] = useState<Mode>(() => {
      return (effectiveUserLevel === 'A1' || effectiveUserLevel === 'A2') ? 'support' : 'challenge';
  });
            ...
            {React.Children.map(children, (child) => {
                if (typeof child === 'string') {
                    ...
                    return child.split(/(\s+)/).map((part, i) => {
                        // Preserve whitespace as string
                        if (part.match(/^\s+$/)) return part;
                        if (part === '') return null;

                        // Logic:
                        // 1. Check if word is difficult for the user
                        // 2. If difficult -> Wrap in SmartWord (clickable)
                        // 3. If easy -> Return plain text (No underline)
                        const isDifficult = levelService.isWordDifficult(part, effectiveUserLevel);

                        if (isDifficult) {
                           return <SmartWord key={i} mode={mode} userLevel={effectiveUserLevel}>{part}</SmartWord>;
                        }
    ...
    <div className="max-w-5xl mx-auto animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
         <div className="flex flex-col">
            <span className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Level: {effectiveUserLevel}</span>
      ...
      <div className={`mb-6 p-4 rounded-lg text-sm border transition-colors duration-300 flex items-start ${
           ...
           <div>
               <span className="font-bold block mb-1">{mode === 'support' ? 'Bilingual Support Active' : 'Immersion Mode Active'}</span>
               {mode === 'support' 
                 ? 'Recommended for your level. Click difficult words for Indonesian translations.' 
                 : 'Recommended for advanced learners. Click difficult words for English definitions.'}
           </div>
       </div>
