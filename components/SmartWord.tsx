...
const SmartWord: React.FC<SmartWordProps> = ({ children, mode }) => {
    ...
    // Check difficulty once on mount/change
    const isDifficult = useMemo(() => {
        return vocabularyService.isRareWord(children);
    }, [children]);
    ...
    useEffect(() => {
        ...
        if (popoverRef.current) {
            try {
                // @ts-ignore - Native Popover API might not be in all TS definitions yet
        ...
        <>
            <span
                ...
                className={`
                    ...
                    hover:bg-indigo-100 hover:text-indigo-700 select-none
                    ${isDifficult 
                        ? 'border-b-2 border-dotted border-indigo-400 hover:border-solid' 
            ...
            <div 
                ...
                // @ts-ignore
                popover="auto"
                className="bg-white rounded-xl shadow-2xl border border-indigo-100 w-80 p-0 text-left text-gray-800 z-50 max-w-[90vw] overflow-hidden"
            >
                 {loading ? (
